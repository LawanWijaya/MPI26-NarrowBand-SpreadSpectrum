...
[selectedIdx, selectedFreq, bestGap] = pickSpreadFaithfulRowsBack(faithfulIdx, K);
...
% In Walsh order, the row index minus 1 is the sequency / sign-change count.
faithfulFreq = faithfulIdx - 1;

% Sort from low to high first. The backward greedy helper will start at the end,
% meaning it starts from the maximum faithful frequency and moves downward.
...
while low <= high
    ...
    % Try to choose K rows by starting at the highest faithful frequency
    % and moving backward, keeping at least gap separation.
...
% Sort only for clean display in the lookup table.
% The actual selection was made from high frequency down to low frequency.
