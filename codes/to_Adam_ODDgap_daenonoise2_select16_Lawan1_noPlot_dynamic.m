...
% Pick 16 faithful rows spread apart in frequency, with odd consecutive gaps
% [selectedIdx, selectedFreq, bestGap, gapVec] = pickSpreadFaithfulRows(faithfulIdx, rowFreq, K);
[selectedIdx, selectedFreq, bestGap, gapVec] = pickSpreadFaithfulRowsDP(faithfulIdx, K);
% Selected Walsh rows
...
disp('Consecutive frequency gaps between selected rows:');
disp(gapVec(:).');
fprintf('Maximum minimum odd frequency gap = %d sign changes\n', bestGap);
...
flag = 1; % to select adam_DAD or dae_DAD
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Improved helper functions for selecting 16 faithful rows
function [selectedIdx, selectedFreq, bestGap, gapVec] = pickSpreadFaithfulRowsDP(faithfulIdx, K)
...
% Use actual Walsh frequency/sign-change count
% faithfulFreq = rowFreq(faithfulIdx);
faithfulFreq = faithfulIdx-1;

% Sort faithful rows by frequency
...
while low <= high
    ...
    chosenIdx = feasibleOddGapDP(faithfulIdxSorted, faithfulFreqSorted, K, gap);
...
if isempty(bestSelectedIdx)
    error('Could not find %d rows satisfying the odd-gap condition.', K);
end
...
gapVec = diff(selectedFreq);

end


function chosenIdx = feasibleOddGapDP(idxSorted, freqSorted, K, gap)
% DP feasibility test:
% Can we select K rows such that each consecutive frequency gap is:
%   1. at least gap
%   2. odd?

m = length(idxSorted);

% dp(t,j) = true if we can select t rows ending at row j
dp = false(K, m);

% parent(t,j) stores the previous row index before j
parent = zeros(K, m);
...
for t = 2:K
    for j = 1:m
        for i = 1:j-1
            ...
            actualGap = freqSorted(j) - freqSorted(i);

            if dp(t-1,i) && actualGap >= gap && mod(actualGap,2) == 1
                dp(t,j) = true;
                parent(t,j) = i;
                break;
            end
...
% Find an endpoint for a valid chain of length K
endIdx = find(dp(K,:), 1, 'last');
...
% Reconstruct selected chain
chain = zeros(1,K);
chain(K) = endIdx;

for t = K:-1:2
    chain(t-1) = parent(t, chain(t));
end

chosenIdx = idxSorted(chain);
